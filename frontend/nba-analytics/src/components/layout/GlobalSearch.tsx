...
export default function GlobalSearch({ onClose }: GlobalSearchProps) {
  ...
  const { data: players } = usePlayerSearch(query, 5)
  const { data: games } = useGameSearch({ q: query, limit: 5 }, query.length >= 2)
  ...
  const hasResults = (players?.length ?? 0) > 0 || (games?.length ?? 0) > 0
      ...
      >
        ...
        {query.length < 2 ? (
          ...
          <div className="max-h-96 overflow-y-auto">
            {(players?.length ?? 0) > 0 && (
              <div>
                ...
                {players!.map(p => (
                  <button
                    key={p.person_id}
                    onClick={() => goToPlayer(p.full_name)}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500">{p.team ?? 'Free Agent'} · {p.position ?? 'N/A'}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {(games?.length ?? 0) > 0 && (
              <div>
                <div className="px-4 py-2 text-xs font-semibold uppercase tracking-widest text-slate-600 border-t border-white/6 mt-1">Games</div>
                {games!.map(g => (
                  <button
                    key={g.game_id}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="text-sm font-medium text-white">{g.away_tricode} @ {g.home_tricode}</div>
                      <div className="text-xs text-slate-500">{fmtDate(g.game_date)} · {g.status}</div>
                    </div>
                    {g.home_score !== undefined && (
                      <div className="text-sm font-mono text-orange-400">{g.away_score} - {g.home_score}</div>
                    )}
                  </button>
                ))}
              </div>
